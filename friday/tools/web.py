...
def register(mcp):
    ...
    async def open_website(url: str) -> str:
        ...
        Use this when the user asks to open a website (e.g., 'Open YouTube', 'Go to github.com').
        If the user provides just a name, construct a likely URL (e.g., 'https://www.youtube.com').
        """
        import webbrowser
        try:
    ...
    @mcp.tool()
    async def operate_browser(task: str) -> str:
        """
        Use this tool to visually navigate and operate ANY website on the user's behalf.
        It spawns a real browser window to search, click, type, and extract data.
        Examples: 'Find flights to Tokyo on Google Flights', 'Check my unread emails on Gmail', 'Buy a mouse on Amazon'.
        Note: This tool takes about 15-30 seconds to complete. You MUST tell the user 'On it, boss. Initializing browser automation...' BEFORE calling it.
        """
        from browser_use import Agent
        from langchain_openai import ChatOpenAI
        import os

        # Patch ChatOpenAI to be compatible with browser-use
        ChatOpenAI.provider = property(lambda self: 'openai')
        ChatOpenAI.model = property(lambda self: self.model_name)

        try:
            llm = ChatOpenAI(model="gpt-4o", api_key=os.getenv("OPENAI_API_KEY"))
            agent = Agent(task=task, llm=llm)
            # The agent.run() method executes the browser automation loop.
            history = await agent.run()
            # history is an AgentHistoryList, we can just return its final result text
            if history and history.is_done():
                return f"Browser task completed. Final Result: {history.final_result()}"
            return "Browser task completed, but no clear final result was returned."
        except Exception as e:
